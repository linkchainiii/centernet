...
def heatmap_detections(out, config):
    ...
    bboxes = np.stack((xs_tl, ys_tl, xs_br, ys_br), axis=3)
...
def remove_by_center_iiem(detections, centers, config):
    ...
    for i in range(config.MAX_NUMS):
        ...
        if np.sqrt(x_size*y_size)> 150:
            n = 5
        tlx = (detection[2] +detection[0])/2-x_size/(2*n)
        tly = (detection[3] +detection[1])/2-y_size/(2*n)
        brx = (detection[2] +detection[0])/2+x_size/(2*n)
        bry = (detection[3] +detection[1])/2+y_size/(2*n)
        proposal_center = centers[centers[:, 2] == detection[5]]
    ...
    if len(result) == 0:
        result.append(np.array([0,0,0,0,0,0]))
